...
const cardVariants = cva(
  "group/card flex flex-col gap-4 overflow-hidden rounded-xl py-4 text-sm has-data-[slot=card-footer]:pb-0 has-[>img:first-child]:pt-0 data-[size=sm]:gap-3 data-[size=sm]:py-3 data-[size=sm]:has-data-[slot=card-footer]:pb-0 *:[img:first-child]:rounded-t-xl *:[img:last-child]:rounded-b-xl",
  {
    variants: {
      tone: {
        /* Neutral surface — the default card across the app. */
        default: "border border-border bg-card text-card-foreground",
        /* Soft mustard tint — B2C plan card. Reads as "this is the choice." */
        tinted: "border border-primary bg-primary-soft text-card-foreground",
        /* Mustard border + lift — B2B Pro plan, "Most Popular" position. */
        highlighted:
          "border-2 border-primary bg-card text-card-foreground shadow-md",
      },
    },
    defaultVariants: {
      tone: "default",
...
function Card({ className, tone = "default", size = "default", ...props }) {
  return (
    <div
      data-slot="card"
      data-tone={tone}
