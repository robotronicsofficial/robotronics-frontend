...
const createOrderCode = () => `SUB-${buildRandomSegment()}`;
...
const normalizePlan = (plan = {}) => ({
  ...
  maxQuizAttemptsPerDay: Number.isFinite(Number(plan?.maxQuizAttemptsPerDay))
    ? Number(plan.maxQuizAttemptsPerDay)
    : 2,
});
...
export const buildSubscriptionCheckout = (checkout = {}) => {
...
export const loadSubscriptionCheckout = () => {
  if (!isBrowser()) {
    return null;
  }
  ...
  try {
    ...
    return buildSubscriptionCheckout(JSON.parse(rawValue));
  } catch {
    return null;
  }
};

export const saveSubscriptionCheckout = (checkout) => {
  if (!isBrowser()) {
    return null;
  }

  const normalizedCheckout = buildSubscriptionCheckout(checkout);
  getStorage()?.setItem(STORAGE_KEY, JSON.stringify(normalizedCheckout));
  return normalizedCheckout;
};

export const updateSubscriptionCheckout = (partialCheckout = {}) => {
  const currentCheckout = loadSubscriptionCheckout();
  return saveSubscriptionCheckout({
    ...currentCheckout,
    ...partialCheckout,
    parent: {
...
export const clearSubscriptionCheckout = () => {
  if (isBrowser()) {
